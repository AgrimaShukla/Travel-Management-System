...
def test_add_booking(mocker, book_package, capsys):
...
def test_display_itinerary(mocker, book_package, capsys):
...
def test_display_not_itinerary(mocker, book_package, capsys):
    book_package.db_access.returning_query.return_value = []
    book_package.display_booking('', '')
    captured = capsys.readouterr()
    assert PrintPrompts.NO_BOOKINGS in captured.out

def test_show_itinerary(mocker, book_package):
...
def test_cancel_booking(mocker, book_package):
